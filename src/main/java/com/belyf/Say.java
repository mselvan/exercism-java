    ...
    public String say(final long number) {
        ...
        if (number < 100) {
            return Optional.ofNullable(NUMBERS.get(number))
                    .orElseGet(() -> NUMBERS.floorEntry(number).getValue() + HYPHEN + this.say(number % 10)); // Find nearest lower number and call the same function again using the mod reduced number
        }
        return this.say(number / NUMBERS.floorKey(number)) + SPACE
                ...
                + (number % NUMBERS.floorKey(number) > 0 ? SPACE + this.say(number % NUMBERS.floorKey(number)) : EMPTY)); // Handle numbers greater than or equal to 100
    }
}